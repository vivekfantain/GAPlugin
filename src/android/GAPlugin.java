		...
		if (action.equals("initGA")) {
			...
			try {
				tracker.send(MapBuilder.createEvent(args.getString(0),
						...
						args.isNull(3) ? null : args.getLong(3)).build());
			...
			try {
				...
				callback.success("trackSocial = " + args.getString(0));
				return true;
			} catch (final Exception e) {
				callback.error(e.getMessage());
			}
			...
			try {
				tracker.send(MapBuilder.createTiming(args.getString(0),
						args.getLong(1),
						args.isNull(2) ? null : args.getString(2),
						args.isNull(3) ? null : args.getString(3)).build());
				callback.success("trackTiming = " + args.getString(0));
				return true;
			} catch (final Exception e) {
				callback.error(e.getMessage());
			}
			...
			try {
				tracker.send(MapBuilder
						.createAppView()
						.set(Fields.customDimension(args.getInt(0)),
								args.getString(1)).build());
